...
void picker_set_positions(ColorPicker *picker, float scale_x, float scale_y)
{
    ...
    if (picker->labels[0]->rect.height > get_height(&picker->color_rect->rect, scale_y))
    {
    ...
    else
    {
        ...
        set_UI_element_position(&picker->labels[0]->rect, picker->rect.x + pad_x, picker->rect.y + get_height(&picker->color_rect->rect, scale_y)/2, 1, 1, MID_LEFT);
    ...
    int y_position = MAX(picker->labels[0]->rect.y + picker->labels[0]->rect.height, picker->color_rect->rect.y + get_height(&picker->color_rect->rect, scale_y));
    ...
    set_UI_element_position(&picker->labels[2]->rect, picker->rect.x + pad_x, picker->sliders[0]->rect.y + get_height(&picker->sliders[0]->rect, scale_y), 1, 1, TOP_LEFT);
    ...
    set_UI_element_position(&picker->labels[3]->rect, picker->rect.x + pad_x, picker->sliders[1]->rect.y + get_height(&picker->sliders[1]->rect, scale_y), 1, 1, TOP_LEFT);
...
int picker_get_height(ColorPicker *picker, float scale_y)
{
    ...
    height += MAX(get_height(&picker->color_rect->rect, scale_y), picker->labels[0]->rect.height);
    ...
    for (i = 0; i < 3; i++)
    {
        height += slider_get_height(picker->sliders[i], scale_y);
    }
