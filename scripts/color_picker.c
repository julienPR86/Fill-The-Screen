...
void picker_set_positions(ColorPicker *picker, float scale)
{
    ...
    else
    {
        ...
        set_UI_element_position(&picker->labels[0]->rect, picker->rect.x, picker->rect.y + get_height(&picker->color_rect->rect, scale)/2, 1, MID_LEFT);
        //Then moves the color rect to the right of the picker title
        set_UI_element_position(&picker->color_rect->rect, picker->labels[0]->rect.x + picker->labels[0]->rect.width * 1.1, picker->rect.y, scale, TOP_LEFT); 
    }
    ...
    set_UI_element_position(&picker->labels[1]->rect, picker->rect.x, y_position, 1, TOP_LEFT);
    ...
    set_UI_element_position(&picker->labels[2]->rect, picker->rect.x, picker->sliders[0]->rect.y + get_height(&picker->sliders[0]->rect, scale), 1, TOP_LEFT);
    ...
    set_UI_element_position(&picker->labels[3]->rect, picker->rect.x, picker->sliders[1]->rect.y + get_height(&picker->sliders[1]->rect, scale), 1, TOP_LEFT);
