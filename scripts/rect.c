...
void render_outline(UI_Element *element, float scale)
{
    ...
    for (int i = 1; i <= element->outline.size; i++)
    {
        ...
        outline_rect.w = (element->width + i * 2) * scale;
        outline_rect.h = (element->height + i * 2) * scale;
...
void render_inline(UI_Element *element, float scale)
{
    ...
    for (int i = 0; i < element->inline_.size; i++)
    {
        ...
        inline_rect.w = (element->width - i * 2) * scale;
        inline_rect.h = (element->height - i * 2) * scale;
